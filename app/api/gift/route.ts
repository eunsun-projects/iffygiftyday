...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const analysisResult = await generateObject({
      ...
      prompt: `이 사진을 보고 다음 정보를 JSON 형식으로 알려줘: is_person (true/false), desc (대상의 묘사), age (예상 나이 숫자). 예시: {"is_person": true, "desc": "귀여운 아이", "age": 6}`,
      messages: [
        {
          role: "user",
          content: [
  ...
  // 최종 응답 구성
  const responsePayload: IffyResponse = {
    age,
    is_person: isPerson,
    desc,
    is_error: isError,
